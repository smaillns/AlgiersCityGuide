...
import smaillns.smail.dzair.adapter.GalleryVideoRecyclerViewAdapter
...
//            columnCount = it.getInt(ARG_COLUMN_COUNT)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // load and show data
        if (mViewState == null || mViewState === smaillns.smail.dzair.view.ViewState.OFFLINE) {
            loadData()
            mViewState = ViewState.CONTENT
        }
        if (mViewState === smaillns.smail.dzair.view.ViewState.CONTENT) {
            if (mDataset != null) renderView()
            showContent()
        }
        if (mViewState === smaillns.smail.dzair.view.ViewState.PROGRESS) {
            showProgress()
        }
        if (mViewState === smaillns.smail.dzair.view.ViewState.EMPTY) {
            showEmpty()
        }
