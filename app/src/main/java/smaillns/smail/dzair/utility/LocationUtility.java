...
public class LocationUtility
{
	...
	public static String getDistanceString(double distance, boolean useMetricSystem)
	{
		...
		if(useMetricSystem)
		{
			if(distance<1000.0d)
			{
				result = String.format(Locale.US, "%d " + "m", (int) distance);
			}
			else if(distance<5000.0d)
			{
				result = String.format(Locale.US, "%.1f " + "Km", distance/1000);
			}
			else
			{
				result = String.format(Locale.US, "%d " + "km", (int) distance/1000);
			}
		}
		else
		{
			...
			{
				result = String.format(Locale.US, "%.2f " + "mile", distanceMiles);
			}
			else if(distanceMiles<10.0d)
			{
				result = String.format(Locale.US, "%.1f " + "mile", distanceMiles);
			}
			else
			{
				result = String.format(Locale.US, "%d " + "mile", (int) distanceMiles);
			}
		}
